...
from networks.photosketch.util import util
from networks.photosketch.util.image_pool import ImagePool
...
# El modelo de arriba, pero al que se le ha retirado todo lo no relacionado con la inferencia
# y se le ha añadido la configuración por defecto.
class Pix2PixInfer(BaseModel):
    def name(self):
        return 'Pix2PixModel'

    def initialize(self): # opt ahora es un diccionario
        # load/define networks
        BaseModel.initialize_inference(self)
        self.netG = networks.define_G(3, 1, 64, "resnet_9blocks", "batch", False, "normal")
        
        self.load_network_inference(self.netG, 'G', 'latest')
        self.netG = self.netG.to(self.device)

        print('Setting up Photosketch model... Done.')

    def set_input(self, input): 
        # Para inferencia la dirección es AtoB
    ...
    # Dado el output de la generadora retorna una lista de imagePil
    def post_processImg(self):
        acum = []
        b,_,_,_ = self.fake_B.size()
        for i in range(b):
            image_numpy = self.fake_B.detach()[i][0].cpu().float().numpy()
            image_numpy = (image_numpy + 1) / 2.0 * 255.0
            image_pil = Image.fromarray(image_numpy.astype(np.uint8))
            acum.append(image_pil)
        return acum
