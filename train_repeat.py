...
# Debido a que tras las 3000 iteraciones mi gpu se queda sin memoria, no pudiendo continuar el entrenamiento, aunque lo puedo reanudar, 
# aquí se busca permitir repetir el entrenamiento un número de veces dado. De modo que, si repeat=4, en total haremos 12000 iteraciones
# reanudando 4 veces. Por no alterar el código, se recomienda un número de epochs alto y limitar entrenamiento con --max_iters, en mi caso 3000. 
# SI no hay limitaciones de memoria usar train.py.
def training_loop():
    opt,parser = get_opt()
    ...
    for path_sketch in pathSketches:
        last_modelG,last_modelDSk,last_modelImg = None,None,None
        for i in range(opt.repeat):
            ...
            if (not last_modelG and opt.pathG and opt.pathDSketch and opt.pathDImage):
                model.resume_training(opt.pathG,opt.pathDSketch,opt.pathDImage)
                log(f,"Continuando entrenamiento...")
            
            if (last_modelG):
                model.resume_training(last_modelG,last_modelDSk,last_modelImg)
                log(f,"Continuando entrenamiento... repetición " + str(i))
            ...
            iters = opt.max_iters * i
            max_iters = opt.max_iters * (i+1)
            ...
            last_modelG = os.path.join(opt.example_folder,path_sketch.split("/")[1], now, "snaps", "G_epoch_{}".format(epoch)) if i!=opt.repeat-1 else None
            last_modelDSk = os.path.join(opt.example_folder,path_sketch.split("/")[1], now, "snaps", "D_sketch_epoch_{}".format(epoch)) if i!=opt.repeat-1 else None
            last_modelImg = os.path.join(opt.example_folder,path_sketch.split("/")[1], now, "snaps", "D_image_epoch_{}".format(epoch)) if i!=opt.repeat-1 else None
            log(f,"Guardando las redes... Hecho\n")
